...
STATUS_HEADER = ["subject", "session", "step", "event", "timestamp", "source", "note"]
STEPS = [
    "ingestion",
    "bids-conversion",
    "fmriprep-process",
    "qa",
    "clpipe-preprocessing",
]
...
def _get_records_latest(records: pd.DataFrame) -> pd.DataFrame:
    latest_records = (
        records.sort_values("timestamp", ascending=False)
        .groupby(["subject", "step"], dropna=False, as_index=False)
        .agg({"timestamp": "max", "event": "first", "note": "first"})
    )
    return latest_records


def _get_records_by_step(records: pd.DataFrame, step=DEFAULT_STEP) -> pd.DataFrame:
    records_by_step = records.loc[records["step"] == step]
    return records_by_step


def _get_records_by_event(records: pd.DataFrame, event=DEFAULT_EVENT) -> pd.DataFrame:
    records_by_event = records.loc[records["event"] == event]
    return records_by_event


def _get_records_pivot(records: pd.DataFrame):
    latest_events_pivot = records.pivot(index="subject", columns="step", values="event")

    return latest_events_pivot


def needs_processing(subjects: list, cache_path: os.PathLike, step=DEFAULT_STEP):
    try:
...
def write_record(
    ...
    with open(cache_path, "a") as cache_file:
        csv_writer = csv.writer(cache_file)
        csv_writer.writerow([subject, session, step, event, timestamp, "", note])
...
def get_latest_by_step(cache_path: os.PathLike):
    ...
    latest_records_pivot_reordered_fillna = latest_records_pivot_reordered.fillna(
        "pending"
    )
...
def show_latest_by_step(
    config_file: os.PathLike = None, cache_path: os.PathLike = None
):
    if not config_file and not cache_path:
        raise ValueError("Need at least one of either config file or cache_path")
