...
def fmri_process_check(config_file, output_file=None, debug=False):
    ...
    sublist = [
        o
        for o in os.listdir(
            resolve_fmriprep_dir_new(config.FMRIPrepOptions.OutputDirectory)
        )
        if os.path.isdir(
            os.path.join(
                resolve_fmriprep_dir_new(config.FMRIPrepOptions.OutputDirectory), o
            )
        )
        and "sub-" in o
    ]
    ...
    for sub in sublist:
        ...
        bold_files = glob.glob(
            os.path.join(
                config.FMRIPrepOptions.BIDSDirectory, sub, "**", "func", "*.nii.gz"
            ),
            recursive=True,
        )
        fmriprep_files = glob.glob(
            os.path.join(
                ...
                "**",
                "func",
                "*" + config.PostProcessingOptions.TargetSuffix,
            ),
            recursive=True,
        )
        ...
        postprocess_files = glob.glob(
            os.path.join(
                config.PostProcessingOptions.OutputDirectory,
                sub,
                "**",
                "func",
                "*" + config.PostProcessingOptions.OutputSuffix,
            ),
            recursive=True,
        )
        ...
        for file in bold_files:
            ...
            row = pd.DataFrame(
                columns=["Subject", "BIDS_File", "FMRIPrep_File", "PostProcessed_File"]
            )

            header = os.path.basename(file).split("_bold.nii.gz")[0]
            logger.debug(header)
            target_fmriprep_file = [
                tfile
                for tfile in fmriprep_files
                if header in tfile
                and config.PostProcessingOptions.TargetSuffix in tfile
            ]
            logger.debug(
                "Finding FMRIPrep file"
                + "[%s]" % ", ".join(map(str, target_fmriprep_file))
            )
            ...
            if target_fmriprep_file:
                ...
                post_proc_file = [
                    pfile
                    for pfile in postprocess_files
                    if header in pfile
                    and config.PostProcessingOptions.OutputSuffix in pfile
                ]
                if post_proc_file:
    ...
    if output_file is None:
        output_file = os.path.join(
            os.path.dirname(os.path.abspath(config_file)), "Checker-Output.csv"
        )
