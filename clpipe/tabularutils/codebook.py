...
@click.option('-file_path', type=click.Path(exists=True, dir_okay=False, file_okay=True),
...
def create_json_codebook(config_file = None, file_path = None, debug = False):
    ...
    if file_path is not None:
        ...
        if config_file is not None and os.path.exists(file_path):
            raise(FileExistsError("You specified both a config file and a target tabular file. Please specify only one of these options."))
    elif config_file is not None:
        file_path = os.path.join(config.config['DICOMToBIDSOptions']['BIDSDirectory'], "participants.tsv")
