...
class ClpipeConfigParser:
    ...
    def setup_project(self, project_title, project_dir, source_data):
        ...
        self.setup_fmriprep_directories(
            os.path.join(self.config["ProjectDirectory"], "data_BIDS"),
            None,
            os.path.join(self.config["ProjectDirectory"], "data_fmriprep"),
        )
        self.setup_roiextract(
            target_dir=os.path.join(
                self.config["ProjectDirectory"], "data_postproc2", "default"
            ),
            target_suffix="desc-postproc_bold.nii.gz",
            output_dir=os.path.join(
                self.config["ProjectDirectory"], "data_ROI_ts", "postproc_default"
            ),
        )
    ...
    def setup_glm(self, project_path):
        ...
        glm_config.config["Level1Setups"][0]["LogDir"] = os.path.join(
            project_path, "logs", "glm_logs", "L1_launch"
        )

        glm_config.config["Level2Setups"][0]["OutputDir"] = os.path.join(
            project_path, "l2_gfeat_folders"
        )
        glm_config.config["Level2Setups"][0]["FSFDir"] = os.path.join(
            project_path, "l2_fsfs"
        )
        glm_config.config["Level2Setups"][0]["LogDir"] = os.path.join(
            project_path, "logs", "glm_logs", "L2_launch"
        )

        glm_config.config_json_dump(project_path, "glm_config.json")
        shutil.copyfile(
            resource_filename("clpipe", "data/l2_sublist.csv"),
            os.path.join(project_path, "l2_sublist.csv"),
        )
    ...
    def setup_fmriprep_directories(self, bidsDir, workingDir, outputDir, log_dir=None):
        ...
        if workingDir is not None:
            self.config["FMRIPrepOptions"]["WorkingDirectory"] = os.path.abspath(
                workingDir
            )
        if outputDir is not None:
            self.config["FMRIPrepOptions"]["OutputDirectory"] = os.path.abspath(
                outputDir
            )
        if log_dir is not None:
            self.config["FMRIPrepOptions"]["LogDirectory"] = os.path.abspath(log_dir)
        else:
            self.config["FMRIPrepOptions"]["LogDirectory"] = os.path.join(
                self.config["ProjectDirectory"], "logs", "FMRIprep_logs"
            )

    def setup_heudiconv(self, dicom_directory, heuristic_file, output_directory):
        if dicom_directory is not None:
            self.config["DICOMToBIDSOptions"]["DICOMDirectory"] = os.path.abspath(
                dicom_directory
            )
        if output_directory is not None:
            self.config["DICOMToBIDSOptions"]["OutputDirectory"] = os.path.abspath(
                output_directory
            )
            os.makedirs(
                self.config["DICOMToBIDSOptions"]["OutputDirectory"], exist_ok=True
            )
        if heuristic_file is not None:
            self.config["DICOMToBIDSOptions"]["HeuristicFile"] = os.path.abspath(
                heuristic_file
            )
    ...
    def setup_dcm2bids(
        ...
        if output_directory is not None:
            self.config["DICOMToBIDSOptions"]["BIDSDirectory"] = os.path.abspath(
                output_directory
            )
        if heuristic_file is not None:
            self.config["DICOMToBIDSOptions"]["ConversionConfig"] = os.path.abspath(
                heuristic_file
            )
        ...
        if log_dir is not None:
            ...
            self.config["DICOMToBIDSOptions"]["LogDirectory"] = os.path.join(
                self.config["ProjectDirectory"], "logs", "DCM2BIDS_logs"
            )

    def setup_bids_validation(self, log_dir=None):
        if log_dir is not None:
            self.config["BIDSValidationOptions"]["LogDirectory"] = os.path.abspath(
                log_dir
            )
        else:
            self.config["BIDSValidationOptions"]["LogDirectory"] = os.path.join(
                self.config["ProjectDirectory"], "logs", "bids_validation_logs"
            )

    def setup_roiextract(self, target_dir, target_suffix, output_dir, log_dir=None):
        if target_dir is not None:
            self.config["ROIExtractionOptions"]["TargetDirectory"] = os.path.abspath(
                target_dir
            )
        if output_dir is not None:
            self.config["ROIExtractionOptions"]["OutputDirectory"] = os.path.abspath(
                output_dir
            )
        if target_suffix is not None:
            self.config["ROIExtractionOptions"]["TargetSuffix"] = target_suffix
        if log_dir is not None:
            self.config["ROIExtractionOptions"]["LogDirectory"] = os.path.abspath(
                log_dir
            )
        else:
            self.config["ROIExtractionOptions"]["LogDirectory"] = os.path.join(
                self.config["ProjectDirectory"], "logs", "ROI_extraction_logs"
            )
    ...
    def update_processing_stream(
        ...
        index = [
            ind
            for ind, e in enumerate(self.config["ProcessingStreams"])
            if e["ProcessingStream"] == stream_name
        ][0]
        if output_dir is not None:
            self.config["ProcessingStreams"][index][target_output][
                "OutputDirectory"
        ...
        if log_dir is not None:
            ...
            self.config["ProcessingStreams"][index][target_output][
                "LogDirectory"
            ] = os.path.join(self.config["ProjectDirectory"], "logs", log_target)
        os.makedirs(self.config[target_output]["LogDirectory"], exist_ok=True)

    def update_runlog(self, subjects, whatran):
        newLog = {
            "DateRan": datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y"),
            "Subjects": subjects,
            "WhatRan": whatran,
