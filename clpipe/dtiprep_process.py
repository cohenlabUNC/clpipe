...
def dti_preprocess(bids_dir=None, working_dir=None, output_dir=None, config_file=None, subjects=None,
                     log_output_dir=None,
    ...
    if not any([config.config['DTIPrepOptions']['BIDSDirectory'], config.config['FMRIPrepOptions']['OutputDirectory'],
    ...
    if log_output_dir is not None:
        ...
        log_output_dir = os.path.join(config.config['DTIPrepOptions']['OutputDirectory'], "Batch_Output")
        os.makedirs(log_output_dir, exist_ok=True)
...
def dti_scan_json_grabber(bids_dir, subject, session):
    ...
    if session is None:
        sub_files_scans = glob.glob(os.path.join(bids_dir, 'sub-{subject}'.format(subject = subject), 'dwi', '*_dwi.nii.gz'),recursive =True)
        sub_files_bvals = glob.glob(
            os.path.join(bids_dir, 'sub-{subject}'.format(subject=subject), 'dwi', '*_dwi.bval'), recursive=True)
        sub_files_bvecs = glob.glob(
            os.path.join(bids_dir, 'sub-{subject}'.format(subject=subject), 'dwi', '*_dwi.bvec'), recursive=True)
    else:
        sub_files_scans = glob.glob(os.path.join(bids_dir, 'sub-{subject}'.format(subject = subject),'ses-{session}'.format(subject = subject), 'dwi', '*_dwi.nii.gz'),recursive =True)
        sub_files_bvals = glob.glob(
            os.path.join(bids_dir, 'sub-{subject}'.format(subject=subject), 'dwi', '*_dwi.bval'), recursive=True)
        sub_files_bvecs = glob.glob(
            os.path.join(bids_dir, 'sub-{subject}'.format(subject=subject), 'dwi', '*_dwi.bvec'), recursive=True)
    ...
    for idx, scan in enumerate(sub_files_scans):
        ...
        if bval_toggle:
            scan_info.append({'scan': sub_files_scans[idx], 'dir': dir[idx], 'trt':total_readout_time[idx], 'bval': scan.split('.')[0]+'.bval'})
        else:
            scan_info.append({'scan': sub_files_scans[idx], 'dir': dir[idx], 'trt': total_readout_time[idx],
                              'bval': None})
...
def topup_command_creator(scan_info, config, subject):
    ...
    for scan in scan_info:
        ...
        if scan['bval'] is not None:
            ...
            maxindex = 1
        fslroi_list.append(fslroi_string.format(
            scan = scan['scan'],
        ...
        for i in range(0,maxindex):
            acqparam_list.append(acq_param_dir_map[scan['dir']].copy().extend(scan['trt']))
