...
def qsiprep_process(bids_dir=None, working_dir=None, output_dir=None, config_file=None, subjects=None,log_dir=None,submit=False, debug=False):
    ...
    if config.config['QSIprepOptions']['TemplateFlowToggle']:
        template1 = "export SINGULARITYENV_TEMPLATEFLOW_HOME={templateflowpath};".format(templateflowpath=config.config["QSIprepOptions"]["TemplateFlowPath"])
        template2 = "${{TEMPLATEFLOW_HOME:-$HOME/.cache/templateflow}}:{templateflowpath},".format(templateflowpath =config.config["QSIprepOptions"]["TemplateFlowPath"])
    else:
        template1 = ""
        template2 = ""
    ...
    if not subjects:
        subjectstring = "ALL"
        sublist = [o.replace('sub-', '') for o in os.listdir(config.config['QSIprepOptions']['BIDSDirectory'])
                   if os.path.isdir(os.path.join(config.config['QSIprepOptions']['BIDSDirectory'], o)) and 'sub-' in o]
    else:
        subjectstring = " , ".join(subjects)
        sublist = subjects

    batch_manager = BatchManager(config.config['BatchConfig'], config.config['QSIprepOptions']['LogDirectory'])
    batch_manager.update_mem_usage(config.config['QSIprepOptions']['QSIprepMemoryUsage'])
