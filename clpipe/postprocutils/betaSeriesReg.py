...
for i in targets:
    ...
    if not os.path.exists(targetRegs):
        ...
        if onsetData.shape[0] is not 0:
            ...
            time_res = TR/16.0
            timeCourse = numpy.arange(0, 32,(TR/16.0))
            ...
            betaSliceShape = orgImageShape[:-1]
            ...
            betaDim = orgImageShape[:-1]+(eventArray.shape[1],)
            betas = numpy.zeros(betaDim)
            for trial in numpy.arange(0, eventArray.shape[1]):
                print(trial)
                regressor = numpy.transpose(numpy.stack([eventArray[:,trial],eventArray.sum(axis = 1) -eventArray[:,trial]], 0))
                ...
                beta = beta.reshape(betaSliceShape)
                betas[:,:,:,trial] = beta
