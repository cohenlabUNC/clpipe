...
def build_postprocessing_workflow(
    ...
    if STEP_SCRUB_TIMEPOINTS in processing_steps:
        ...
        postproc_wf.connect(
            input_node, "confounds_file", mult_scrub_wf, "inputnode.confounds_file"
        )
        postproc_wf.connect(
            mult_scrub_wf, "outputnode.out_file", image_wf, "inputnode.scrub_vector"
        )
        postproc_wf.connect(
            mult_scrub_wf, "outputnode.out_file", confounds_wf, "inputnode.scrub_vector"
        )
...
def build_image_postprocessing_workflow(
    ...
    for index, step in enumerate(processing_steps):
        ...
        if step == STEP_TEMPORAL_FILTERING:
            ...
            insert_na = postprocessing_config["ProcessingStepOptions"][
                "ScrubTimepoints"
            ]["InsertNA"]
