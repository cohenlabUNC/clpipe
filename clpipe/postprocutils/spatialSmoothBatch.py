...
parser.add_argument("--script_dir", help="log file directory name within derivatives, will create a directory if directory doesn't exist (default: spatialSmoothScripts)", default = "spatialSmoothScripts")
...
if not os.path.exists(args.input_BIDS + "/derivatives/" + args.script_dir):
    os.makedirs(args.input_BIDS + "/derivatives/" + args.script_dir)
...
for i in targets:
    batchHeader = '''#!/bin/bash
    # SBATCH --job-name={jobName}SpatSmooth
    # SBATCH --ntasks=4
    # SBATCH --time=24:0:0
    # SBATCH --mem=8000\n'''.format(jobName=i)
    ...
    if os.path.exists(outputFile) and args.overwrite:
        ...
        shPipe = open(args.input_BIDS + "/derivatives/" + args.script_dir + "/" + name + ".sh", 'w')
        shPipe.write(batchHeader)
        shPipe.write(fslCommand)
        shPipe.close()
