...
def spec_inter(arr, tr, ofreq, scrub_mask, hifreq, binSize=5000):
    ...
    offsets = numpy.arctan2(numpy.sin(numpy.matmul(numpy.transpose(2 * freqang), tobs_good)).sum(1),
                            numpy.cos(numpy.matmul(numpy.transpose(2 * freqang), tobs_good)).sum(1)) / (2 * freqang)
    costerm = numpy.cos(numpy.matmul(numpy.transpose(freqang), tobs_good) - (offsets * freqang).T)
    ...
    for bin in range(0, totbins):
        ...
        cosMultTemp = [numpy.matmul(costerm[:, i][numpy.newaxis].T, gooddata[i, :][numpy.newaxis]) for i in
                       range(0, tobs_good.shape[1])]
        cosmult = numpy.dstack(cosMultTemp)
        num = cosmult.sum(axis=2)
        dem = (numpy.power(costerm, 2)).sum(axis=1)[numpy.newaxis].T
        cosine = num / dem
        sinMultTemp = [numpy.matmul(sinterm[:, i][numpy.newaxis].T, gooddata[i, :][numpy.newaxis]) for i in
                       range(0, tobs_good.shape[1])]
