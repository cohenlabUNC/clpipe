...
def fmri_postprocess(
    ...
    config.setup_postproc(
        target_dir, target_suffix, output_dir, output_suffix, beta_series, log_dir
    )
    ...
    if beta_series:
        raise ValueError(
            "At this time, the beta series functionality is no longer working due to incompatibilities between packages."
        )
    ...
    if processing_stream is not None:
        ...
        processing_stream_config = [
            i
            for i in processing_stream_config
            if i["ProcessingStream"] == processing_stream
        ]
    ...
    if alt_proc_toggle:
        if beta_series:
            config.update_processing_stream(
                processing_stream,
                processing_stream_config[0]["BetaSeriesOptions"]["OutputDirectory"],
                processing_stream_config[0]["BetaSeriesOptions"]["OutputSuffix"],
                processing_stream_config[0]["BetaSeriesOptions"]["LogDirectory"],
            )
            config.config["BetaSeriesOptions"].update(
                processing_stream_config[0]["BetaSeriesOptions"]
            )
        else:
            config.config["PostProcessingOptions"].update(
                processing_stream_config[0]["PostProcessingOptions"]
            )
            config.update_processing_stream(
                processing_stream,
                processing_stream_config[0]["PostProcessingOptions"]["OutputDirectory"],
    ...
    if not subjects:
        ...
        sublist = [
            o.replace("sub-", "")
            for o in os.listdir(config.config[output_type]["TargetDirectory"])
            if os.path.isdir(
                os.path.join(config.config[output_type]["TargetDirectory"], o)
            )
            and "sub-" in o
        ]
    else:
    ...
    submission_string = (
        """fmri_postprocess -config_file={config} -target_dir={targetDir} -target_suffix={targetSuffix} """
    ...
    if batch:
        config_string = config.config_json_dump(
            config.config[output_type]["OutputDirectory"], os.path.basename(config_file)
        )
        batch_manager = BatchManager(
            config.config["BatchConfig"], config.config[output_type]["LogDirectory"]
        )
        batch_manager.update_mem_usage(
            config.config["PostProcessingOptions"]["PostProcessingMemoryUsage"]
        )
        batch_manager.update_time(
            config.config["PostProcessingOptions"]["PostProcessingTimeUsage"]
        )
        batch_manager.update_nthreads(
            config.config["PostProcessingOptions"]["NThreads"]
        )
        ...
        for sub in sublist:
            ...
            batch_manager.add_job(Job("PostProcessing" + sub, sub_string_temp))
        if submit:
...
def _fmri_postprocess_subject(
    config, subject, task, logger, tr=None, beta_series=False
):
    if beta_series:
        output_type = "BetaSeriesOptions"
    else:
    ...
    search_string = os.path.abspath(
        os.path.join(
            config.config[output_type]["TargetDirectory"],
            "sub-" + subject,
            "**",
    ...
    for image in subject_files:
        if task is None or "task-" + task in image:
            ...
            try:
                ...
                if config.config["PostProcessingOptions"]["DropCSV"] is not "":
                    temp = drop_tps[
                        drop_tps["file_name"].str.match(os.path.basename(image))
                    ]["TR_round"]
                    if len(temp) is 1:
                        tps_drop = int(temp)
                        logger.info(
                            "Found drop TP info, will remove last "
                            + str(tps_drop)
                            + " time points"
                        )
                else:
                    tps_drop = None
                _fmri_postprocess_image(
                    config, image, logger, task, tr, beta_series, tps_drop
                )
...
def _fmri_postprocess_image(
    config, file, logger, task=None, tr=None, beta_series=False, drop_tps=None
):
    confound_regressors = _find_confounds(config, file)
    output_file_path = _build_output_directory_structure(
        config, file, logger, beta_series
    )
    ...
    if not os.path.exists(confound_regressors):
        logger.warning(
            "Could not find a confound file for " + file + ". Moving onto next scan"
        )
        return
    else:
    ...
    if not beta_series:
        ...
        if config.config["PostProcessingOptions"]["Scrubbing"]:
            ...
            scrubTargets = get_scrub_vector(
                fdts, fd_thres, scrub_behind, scrub_ahead, scrub_contig
            )
        ...
        if scrub_toggle and filter_toggle:
            ...
            logger.debug(
                "Memory Usage Before Spectral Interpolation:"
                + str(psutil.virtual_memory().total >> 30)
                + " GB"
            )
            data = spec_inter(
                data,
                ...
                scrubTargets,
                hfreq,
                binSize=config.config["PostProcessingOptions"][
                    "SpectralInterpolationBinSize"
                ],
            )
        ...
        logger.debug(
            "Memory Usage After Spectral Interpolation GC:"
            + str(psutil.virtual_memory().total >> 30)
            + " GB"
        )
        ...
        if scrub_toggle:
            ...
            toOut = numpy.column_stack(
                [
                    numpy.arange(1, len(scrubTargets) + 1, 1),
                    numpy.asarray(scrubTargets),
                    fdts,
                    orig_fdts,
                ]
            )
            logger.info(
                "Saving Scrub Targets to "
                + os.path.join(
                    os.path.dirname(output_file_path), sans_ext + "_scrubTargets.csv"
                )
            )
            numpy.savetxt(
                os.path.join(
        ...
        if os.path.exists(events_file):
            confounds, fdts = _regression_prep(
                config, confound_regressors, beta_series, logger
            )
            ntp = len(confounds)
            if tr is None:
                image_json_path = _find_json(config, file, logger)
            ...
            if hp > 0 or lp > 0:
                ...
                filt = calc_filter(hp, lp, tr, order)
                confounds = apply_filter(filt, confounds)
            filt_ev_array, valid_events = _ev_mat_prep(
                events_file, filt, tr, ntp, beta_series_options, logger
            )
            ...
            events_output = (
                os.path.splitext(os.path.splitext(output_file_path)[0])[0]
                + "_usedevents.tsv"
            )
...
def _ev_mat_prep(event_file, filt, TR, ntp, config_block, logger):
    ...
    valid_trials = [
        ind
        for ind, x in enumerate(trial_types)
        if x not in [config_block["ExcludeTrialTypes"]]
    ]
    ...
    for index, row in valid_events.iterrows():
        ...
        index1 = numpy.logical_and(
            (time_up >= row["onset"]), (time_up <= row["onset"] + row["duration"])
        )
        ev_loop[index1] = 1
...
def _beta_series_calc(data, filt_ev_mat, filt_confound_mat, logger):
    ...
    for index in range(filt_ev_mat.shape[1]):
        ...
        temp_mat = numpy.concatenate(
            [
                numpy.expand_dims(filt_ev_mat[:, index], 1),
                numpy.expand_dims(numpy.sum(filt_ev_mat, 1) - filt_ev_mat[:, index], 1),
                filt_confound_mat,
            ],
            1,
        )
...
def _regression_prep(config, confound_filepath, logger):
    ...
    if len(config.config["PostProcessingOptions"]["Confounds"]) > 0:
        cons_re = [
            re.compile(regex_wildcard(co))
            for co in config.config["PostProcessingOptions"]["Confounds"]
        ]
        target_cols = []
        for reg in cons_re:
            logger.debug(
    ...
    if len(config.config["PostProcessingOptions"]["ConfoundsQuad"]) > 0:
        cons_re = [
            re.compile(regex_wildcard(co))
            for co in config.config["PostProcessingOptions"]["ConfoundsQuad"]
        ]
        target_cols = []
        for reg in cons_re:
            target_cols.extend(
        ...
        confounds_mat = pandas.concat(
            [confounds_mat, confounds_quad_mat], axis=1, ignore_index=True
        )
        logger.debug(str(confounds_mat.shape))
    if len(config.config["PostProcessingOptions"]["ConfoundsDerive"]) > 0:
        cons_re = [
            re.compile(regex_wildcard(co))
            for co in config.config["PostProcessingOptions"]["ConfoundsDerive"]
        ]
        target_cols = []
        for reg in cons_re:
            target_cols.extend(
        ...
        confounds_mat = pandas.concat(
            [confounds_mat, confounds_lagged_mat], axis=1, ignore_index=True
        )
        logger.debug(str(confounds_mat.shape))
    ...
    if len(config.config["PostProcessingOptions"]["ConfoundsQuadDerive"]) > 0:
        cons_re = [
            re.compile(regex_wildcard(co))
            for co in config.config["PostProcessingOptions"]["ConfoundsQuadDerive"]
        ]
        target_cols = []
        for reg in cons_re:
            target_cols.extend(
        ...
        confounds_mat = pandas.concat(
            [confounds_mat, confounds_qlagged_mat], axis=1, ignore_index=True
        )
        logger.debug(str(confounds_mat.shape))
...
def _find_json(config, filepath, logger):
    ...
    jsons = glob.glob(
        os.path.join(config.config["FMRIPrepOptions"]["BIDSDirectory"], "**", "*.json"),
        recursive=True,
    )
    logger.debug(jsons)
    ...
    sub_level_json = [
        json for json in jsons if "_".join(components[0:2]) + "_bold.json" in json
    ]
    ...
    scan_level_json = [
        json for json in jsons if "_".join(components[0:3]) + "_bold.json" in json
    ]
    ...
    if len(scan_level_json) is not 0:
        ...
        scan_level_json = [
            json for json in jsons if "_".join(components[0:4]) + "_bold.json" in json
        ]
        if len(scan_level_json) is not 0:
            target_json = scan_level_json[0]
        else:
            scan_level_json = [
                json
                for json in jsons
                if "_".join(components[0:5]) + "_bold.json" in json
            ]
            if len(scan_level_json) is not 0:
                target_json = scan_level_json[0]
...
def _find_confounds(config, filepath):
    ...
    return os.path.join(
        ...
        root_file + config.config["PostProcessingOptions"]["ConfoundSuffix"],
    )
...
def _build_output_directory_structure(
    config, filepath, logger, beta_series_toggle=False
):
    output_type = "PostProcessingOptions"
    ...
    target_directory = os.path.join(
        config.config[output_type]["OutputDirectory"], target_directory
    )
...
def _notch_filter_fd(config, confounds_filepath, tr, drop_tps=None):
    ...
    confounds = numpy.array(
        confounds[config.config["PostProcessingOptions"]["MotionVars"]]
    )
