...
def convert2bids(
    dicom_dir=None,
    dicom_dir_format=None,
    bids_dir=None,
    conv_config_file=None,
    config_file=None,
    overwrite=False,
    clear_cache=False,
    clear_outputs=False,
    ...
    if subject and not subjects:
        subjects = [subject]
        logger.warn(
            "WARNING: The -subject option is deprecated. "
    ...
    if dcm2bids:
        ...
        heudiconv_wrapper(
            ...
            dicom_dir=config.convert2bids.dicom_directory,
            submit=submit,
            output_directory=config.convert2bids.bids_directory,
            heuristic_file=config.convert2bids.conversion_config,
            overwrite=overwrite,
            batch_manager=batch_manager,
            logger=logger,
            dicom_dir_format=config.convert2bids.dicom_format_string,
...
def dcm2bids_wrapper(
    ...
    sub_sess_list, folders = _get_sub_session_list(
        dicom_dir, dicom_dir_format, logger, subjects=subjects, session=session
    )

    if len(sub_sess_list) == 0:
        logger.warn(
            (
                f"There were no subjects/sessions found for format "
    ...
    if status_cache:
        ...
        subjects_need_processing = needs_processing(subjects_to_process, status_cache)
...
def heudiconv_wrapper(
    ...
    sub_sess_list, folders = _get_sub_session_list(
        dicom_dir, dicom_dir_format, logger, session=session, subjects=subjects
    )
    ...
    if session:
        if "{session}" not in dicom_dir_format:
            raise ValueError(
                "Session value given but no '{session}' placeholder found in dicom_dir_format"
            )
...
def _get_sub_session_list(
    dicom_dir, dicom_dir_format, logger, subjects=None, session=None
):
    ...
    if subjects:
        sub_inds = [
            ind for ind, x in enumerate(sub_sess_list) if x["subject"] in subjects
        ]
    if session is not None:
        sess_inds = [
            ind for ind, x in enumerate(sub_sess_list) if x["session"] == session
        ]
...
@click.option(
    ...
    required=True,
    default=None,
    help="A subject that has all scans of interest present.",
)
@click.option(
    "-session", default=None, help="A session indicator, if sessions are present"
)
@click.option(
    "-dicom_directory",
    required=True,
    help="The specially formatted dicom directory string. Please see help pages at https://clpipe.readthedocs.io/en/latest/index.html for more details",
)
@click.option(
    "-output_file", default="dicom_info.tsv", help="The dicom info output file name."
)
@click.option("-submit", is_flag=True, default=False, help="Submission job to HPC.")
def dicom_to_nifti_to_bids_converter_setup(
    subject=None,
    session=None,
