...
def test_glm_prepare_controller_L1(glm_config_file: Path):
    ...
    with pytest.raises(SystemExit) as e:
        glm_prepare.glm_prepare(glm_config_file=glm_config_file, level="L1", model="example")
...
def test_glm_prepare_controller_L1_old_config(old_glm_config_file: Path):
    ...
    # Monkeypatch helper function so that controller can be tested in isolation
    glm_prepare._glm_l1_propagate = _old_glm_l1_propagate_return
...
def _old_glm_l1_propagate_return(l1_block, task_name, reference_image, logger):
    """Do nothing except check some config values."""
...
def test_glm_prepare_controller_L2(glm_config_file: Path):
    ...
    with pytest.raises(SystemExit) as e:
        glm_prepare.glm_prepare(glm_config_file=glm_config_file, level="L2", model="example")
...
def test_glm_prepare_controller_L2_old_config(old_glm_config_file: Path):
    ...
    # Monkeypatch helper function so that controller can be tested in isolation
    glm_prepare._glm_l2_propagate = _old_glm_l2_propagate_return
...
def _old_glm_l2_propagate_return(l2_block, reference_image, logger):
    """Do nothing except check some config values."""
